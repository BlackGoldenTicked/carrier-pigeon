...
function setupMessageListener() {
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    console.log('[Gemini] 收到消息:', message)
...
async function handleFillText(text: string, autoSend: boolean = false) {
  try {
    console.log(`[Gemini] 开始填充文本: "${text}", 自动发送: ${autoSend}`)
...
async function handleAutoSend() {
  try {
    ...
    button.click()
    console.log('[Gemini] 自动发送成功')
    
    return { success: true, message: '自动发送成功' }
  } catch (error) {
    console.error('[Gemini] 自动发送失败:', error)
    throw error
  }
}
