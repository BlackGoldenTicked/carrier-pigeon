  ...
  return new Promise((resolve) => {
    ...
    function check() {
      for (const selector of selectors) {
        ...
        for (const element of elements) {
          if (isElementVisible(element)) {
            console.log(`[Kimi] 找到可见元素:`, selector, element)
            resolve(element)
            return
          }
        }
      ...
      if (Date.now() - startTime > timeout) {
        console.log(`[Kimi] 等待元素超时:`, selectors)
        resolve(null)
        return
      }
...
function setupMessageListener() {
  console.log('[Kimi] 设置消息监听器')
  
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    console.log('[Kimi] 收到消息:', message)
    
    if (message.action === 'fillText') {
      handleFillText(message.text, message.autoSend || false)
        .then(result => {
          console.log('[Kimi] 填充文本结果:', result)
          sendResponse(result)
        })
        .catch(error => {
          console.error('[Kimi] 填充文本错误:', error)
    ...
    if (message.action === 'autoSend') {
      handleAutoSend()
        .then(result => {
          console.log('[Kimi] 自动发送结果:', result)
          sendResponse(result)
        })
        .catch(error => {
          console.error('[Kimi] 自动发送错误:', error)
          sendResponse({ success: false, error: error.message })
        })
      return true
    }
    ...
    if (message.action === 'fillAndSend') {
      handleFillAndSend(message.text)
        .then(result => {
          console.log('[Kimi] 填充并发送结果:', result)
          sendResponse(result)
        })
        .catch(error => {
          console.error('[Kimi] 填充并发送错误:', error)
          sendResponse({ success: false, error: error.message })
        })
      return true
    }
    ...
    if (message.action === 'getDebugInfo') {
      ...
      console.log('[Kimi] 调试信息:', debugInfo)
...
async function handleFillText(text: string, autoSend: boolean = false) {
  try {
    console.log(`[Kimi] 开始填充文本: "${text}", 自动发送: ${autoSend}`)
    ...
    console.error('[Kimi] 填充文本失败:', error)
    throw error
  }
}
...
async function handleFillAndSend(text: string) {
  try {
    console.log(`[Kimi] 开始填充并发送: "${text}"`)
    
    await handleFillText(text, false)
    await new Promise(resolve => setTimeout(resolve, 500))
    return await handleAutoSend()
  } catch (error) {
    console.error('[Kimi] 填充并发送失败:', error)
    throw error
  }
}
...
console.log('[Kimi] Content script已加载')