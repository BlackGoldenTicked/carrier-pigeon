...
export const config: PlasmoCSConfig = {
  matches: ['https://tongyi.aliyun.com/*', 'https://qianwen.aliyun.com/*'],
  run_at: 'document_end'
}
...
async function handleFillText(text: string, autoSend: boolean = false) {
  try {
    console.log(`[通义千问] 开始填充文本: "${text}", 自动发送: ${autoSend}`)
    ...
    console.error('[通义千问] 填充文本失败:', error)
    throw error
  }
}
...
async function handleAutoSend() {
  try {
    ...
    button.click()
    console.log('[通义千问] 自动发送成功')
    
    return { success: true, message: '自动发送成功' }
  } catch (error) {
    console.error('[通义千问] 自动发送失败:', error)
    throw error
  }
}
